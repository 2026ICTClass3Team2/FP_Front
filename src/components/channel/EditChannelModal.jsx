...
const EditChannelModal = ({ isOpen, onClose, onSuccess, channel }) => {
  ...
  useEffect(() => {
    if (isOpen && channel) {
      ...
      setSelectedTechStacks(channel.techStacks || []);
      setError('');
    }
  }, [isOpen, channel]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await jwtAxios.put(`/channels/${channel.channelId}`, formData, {
      ...
      setError(err.response?.data?.message || '채널 수정 중 오류가 발생했습니다.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <>
      <Modal isOpen={isOpen} onClose={onClose} title="채널 수정">
